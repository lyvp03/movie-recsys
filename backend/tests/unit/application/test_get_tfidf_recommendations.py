...
def use_case(mock_repo, mock_vector_store):
    return GetTFIDFRecommendations(mock_repo, mock_vector_store)


def test_execute_success(use_case, mock_repo, mock_vector_store):
    results = use_case.execute(1, top_k=2)
    
    # It should ask for vector of movie 1
    mock_vector_store.get_vector.assert_called_once_with("movies_tfidf", 1)
    
    # It should ask for top_k + 1 (to filter out self)
    mock_vector_store.search.assert_called_once_with(
        collection="movies_tfidf", vector=[0.1, 0.2], top_k=3
    )
...
def test_execute_excludes_self(use_case, mock_vector_store):
    # Even if self is not the first result, it should be excluded
    mock_vector_store.search.return_value = [
        SearchResult(2, 0.9, {}),
        SearchResult(1, 0.85, {}), # Self is 2nd
        SearchResult(3, 0.8, {})
    ]
