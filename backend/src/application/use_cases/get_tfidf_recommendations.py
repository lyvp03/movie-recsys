...
# Can be overridden by env vars, but default to 'movies_tfidf'
TFIDF_COLLECTION = os.getenv("QDRANT_COLLECTION_TFIDF", "movies_tfidf")


class GetTFIDFRecommendations(IRecommender):
    ...
    def execute(self, movie_id: int, top_k: int = 10) -> list[RecommendationDTO]:
        ...
        vector = self._vector_store.get_vector(TFIDF_COLLECTION, movie_id)
        
        # 3. Search for nearest neighbors. Ask for top_k + 1 to account for the movie itself.
        search_results = self._vector_store.search(
            collection=TFIDF_COLLECTION,
            vector=vector,
            top_k=top_k + 1,
        )
